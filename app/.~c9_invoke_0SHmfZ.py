...
def userDetails(user_id):
    try:
        ...
        current = {"id": user.id, "firstname": user.firstname, "lastname": user.lastname, "email": user.email, """user_photo": os.path.join(app.config['GET_FILE'], user.profile_photo), "biography": user.biography, "joined": user.joined_on.strftime("%b %Y"), "postNum": len(user.posts), "followers": len(user.followers), "posts": []}
